...
class DataReader():
    ...
    def __init__(self, datasets, kpModel, output_path):
        ...
        for dataset in datasets:
            ...
            path = os.path.normpath(f"output_reduced/{dataset}--{kpModel}.hdf5")
    ...
    def deleteSelectedVideosToBan(self):
        ...
        df_selectedBanned = pd.read_csv("banned_selected_videos.csv", header=None)
    ...
    def saveData(self, indexOrder, train=True):
        ...
        # set the path
        save_path = os.path.normpath(f"split_reduced/{self.output_path.split(os.sep)[1]}")
...
dataset_out_name = [dataset for dataset in datasets]
dataset_out_name = '-'.join(dataset_out_name)
print(dataset_out_name)
print(f"procesing {datasets} - using {kpModel} ...")

output_path = f"output_reduced/{dataset_out_name}--{CLASS_NUM}--{kpModel}.hdf5"
